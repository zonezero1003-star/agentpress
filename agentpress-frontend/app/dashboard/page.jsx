...
export default function Dashboard() {
  ...
  async function reviewDeployment() {
    ...
    try {
      ...
      if (!bId) {
        const business = await api.createBusiness({ name: businessName });
        bId = business.id;
        setBusinessId(bId);
      }
      if (!aId) {
          ...
          <div className="flex-1 min-w-0">
            ...
            {step === 'overview' && (
              <div>
                ...
                <div className="mb-5 p-3 rounded-lg text-xs f-mono" style={{ background: 'rgba(14,165,233,0.06)', border: `1px solid ${SKY}`, color: SKY_DEEP }}>
                  ⏱ OKX's Web3 Developer Challenge submission (Google Form + X post) closes Jul 17, 23:59 UTC — separate from the routine 24h service review. See web3.okx.com/xlayer/build-x-series
                </div>
            ...
            {step === 'configure' && (
              <div>
                ...
                <div className="flex justify-between">
                  ...
                  <button disabled={!businessName || !agentName || loading} onClick={reviewDeployment}
                    className="f-mono text-sm px-4 py-2 rounded-md flex items-center gap-1"
                    style={{ background: businessName && agentName ? SKY : LINE, color: businessName && agentName ? '#fff' : SUB }}>
